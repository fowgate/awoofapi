...
exports.startCron = () => {
  console.log("yes yes i'm working as a lambda function!!!");
  giveaway();
  makePayments();
};
...
async function setWinners(giveaway_id, num) {
  ...
  const participants = [];
  for(var i=0; i<num; i++){
    //indexArray= [...indexArray, Math.floor(Math.random() * pre_participants.length)]
    [...participants, pre_participants[Math.floor(Math.random() * pre_participants.length)]]
  }
...
//"db": "mongodb+srv://awoof:awoof1234@awoofcluster-tetxw.mongodb.net/awoof?retryWrites=true&w=majority"
